...
def get_task(SITE,taskID):
    ...
    #status = '[{}][{}]'.format(colored(x,'cyan',attrs=['bold']),colored(SITE.title() + '-' + taskID,'white',attrs=["bold"]))
    status = '[{}{}{}]'.format(Fore.CYAN + Style.BRIGHT + x, Fore.WHITE + '][',Fore.WHITE + SITE.title() + '-' + taskID)
    return status
