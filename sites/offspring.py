...
# 
    ...
    def addToCart(self):
        ...
        if 'false' in capCheck.text:
            ...
            if captchaResponse == "empty":
                captchaResponse = captcha.v3('6Ld-VBsUAAAAABeqZuOqiQmZ-1WAMVeTKjdq2-bJ',self.task["PRODUCT"],self.proxies,SITE,self.taskID)
    ...
    def delivery(self):
        ...
        deliveryPayload = {
            "deliveryModeCode": "standarduk",
            "CSRFToken": self.csrf
        }

        try:
            deliveryPageResponse = self.session.get('https://www.offspring.co.uk/checkout/singlePageCheckout', data=deliveryPayload,headers={
                'user-agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/87.0.4280.88 Safari/537.36',
                'accept': '*/*',
                'referer':'https://www.offspring.co.uk/view/content/basket',
                'accept-encoding': 'gzip, deflate, br',
                'accept-language': 'en-US,en;q=0.9',
                'cookie':'CSRFToken=' + self.csrf
